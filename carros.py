...
def cadastrar_carro_para_cliente(cliente_id, nome_cliente):
    ...
    linha = f"{novo_id}|{cliente_id}|{modelo}|{cor}|{placa}"
    salvar_dados('carros', linha)
...
def listar_carros():
    ...
    if not conteudo['carros']:
        print("Nenhum carro cadastrado.")
        return
    
    for carro in conteudo['carros']:
        print(f"ID: {carro['id']} -  {carro['modelo']} - Placa: {carro['placa']} - Cor: {carro['cor']} Id_Cliente: {carro['id_cliente']}")
